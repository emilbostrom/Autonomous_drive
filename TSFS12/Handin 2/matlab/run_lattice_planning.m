...
cost_to_go = @(x, xg) norm(world.st_sp(1:2, x) - world.st_sp(1:2, xg)) + abs(world.st_sp(3, x)-world.st_sp(3, xg));
...
for i = 1:5
    [p, sp] = mp.plan_to_path(start,plan{i});
    figure()
    draw_map_obstacles(world,start,goal,start_arrow,goal_arrow,arrow_width)
    hold on
    for k = 1:length(sp)
        segment_color = "b";
        if sp(k, 1) == -1
...
% hold on
% for i = 1:5
%    plot((mean([plan{1}{i}.num_expanded_nodes,plan{2}{i}.num_expanded_nodes,plan{3}{i}.num_expanded_nodes,plan{4}{i}.num_expanded_nodes])),mean([plan{1}{i}.time,plan{2}{i}.time,plan{3}{i}.time,plan{4}{i}.time]),'*')
% end
% xlabel('nr of expanded nodes')
% ylabel('time')
% title('Average for 4 different worlds')
% legend(plan{1}{1}.name,plan{1}{2}.name,plan{1}{3}.name,plan{1}{4}.name,plan{1}{5}.name)

%%
function draw_map_obstacles(world,start,goal,start_arrow,goal_arrow,arrow_width)
    clf()
    world.draw()
    hold on
    axis([world.xmin, world.xmax, world.ymin, world.ymax])
